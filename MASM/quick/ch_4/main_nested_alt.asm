...
inloop: mov     al, byte ptr [R12]     ; Load byte from input.
        mov     byte ptr echobuf, al   ; Store to echo buffer
        mov     ax, word ptr newln     ; Load word (2 bytes) (CRLF).
        mov     word ptr echobuf+1, ax ; Store to echo buffer.

        mov     RCX, stdout     ; Handle to standard output device
        lea     RDX, echobuf    ; Point buffer containing single char plus CR and LF.
        mov     R8, 3           ; Length of char + CR/LF buffer.
        lea     R9, nbwr        ; Number of bytes actually written.
        call    WriteConsoleA   ; Write text string to command box.
        inc     R12             ; Set pointer to next character.
        cmp     byte ptr [R12], 0AH ; Compare the next char in R12 to LF
        jne     inloop          ; Loop until message complete (R12 points to LF).
...
echobuf byte    MaxBuf DUP (?)  ; Memory buffer for single char and CR LF.
